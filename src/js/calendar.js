...
export const getMonthData = () => {
    ...
    return dateObjs.map(date => {
        return {
            ...
            firstDayIdx: new Date(date.getFullYear(), date.getMonth(), 1).getDay() - 1,
        };
    });
};
...
export const createCalendarMonth = (monthData, calendarMonthEls) => {
    ...
    calendarMonthEls.forEach((calendarMonthEl, i) => {
        const month = getCalendarMonth(monthData[i].numberOfDays, 7, monthData[i].firstDayIdx);
        const calendarGridEl = calendarMonthEl.querySelector('.booking__calendar-grid');
        ...
        month.map((week, weekIdx) => {
            week.map((dayObj, dayIdx) => {
                ...
                if (!dayObj.notCurrentMonth){
                    calendarDayLiEl.classList.add('calendar-item', 'booking__calendar-number');
...
export const monthClickEventListener = (e) =>  {
    ...
    if (e.target.classList.contains('booking__calendar-number')){
        ...
        if (!isFirstDateClicked) {
            firstSelectedDate = new Date(currentYear, currentMonth - 1, parseInt(e.target.textContent));
            checkInInput.value = `${currentDay.padStart(2, '0')}.${currentMonth.toString().padStart(2, '0')}.${currentYear}`;
            ...
            secondSelectedDate = new Date(currentYear, currentMonth - 1, parseInt(e.target.textContent));
            ...
            if (secondSelectedDate < firstSelectedDate) {
                ...
                checkOutInput.value = `${currentDay.padStart(2, '0')}.${currentMonth.toString().padStart(2, '0')}.${currentYear}`;
                console.dir(firstSelectedDateEl);
                colorDatesBetweenSelected(firstSelectedDateEl, secondSelectedDateEl, '#F3F3F4');
            }
        } else if (isFirstDateClicked && isSecondDateClicked) {

            colorDatesBetweenSelected(firstSelectedDateEl, secondSelectedDateEl, 'inherit');
...
const colorDatesBetweenSelected = (start, end, color) => {
    ...
    while (currentEl && !(currentEl === end)) {
        ...
        if (currentEl.nextElementSibling) {
            ...
            currentEl = months[1].querySelector('.booking__calendar-grid').lastElementChild.firstElementChild;
        }
    }
}