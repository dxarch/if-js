...
test('Student constructor throws an error if required fields are missing', () => {
    expect(() => new Student({firstName: 'James', lastName: 'Smith'})).toThrow('Required properties are missing!');
});

test('Student inherits from User class', () => {
    expect(Student instanceof User).toBe(true);
});

describe('Student: James Smith, 2016, Machine Learning', () => {
    const student = new Student({firstName: 'James', lastName: 'Smith', admissionYear: 2016, courseName: 'Machine Learning'});
