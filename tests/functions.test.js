...
test('Parse date throws an error in case of wrong input', () => {
  expect(() => parseDate('2020.11.26')).toThrow('Input date doesn\'t match format!');
});

test('"Hotel" query returns ["Russia", "Saint-Petersburg, "Hotel Leopold", "Marocco", "Ourika", "Rokoko Hotel", ' +
    '"Germany", "Berlin", "Hotel Rehberge Berlin Mitte"]', () => {
  expect(findDataByQuery('Hotel')).toContain("Russia", "Saint-Petersburg",
      "Hotel Leopold", "Marocco", "Ourika", "Rokoko Hotel","Germany", "Berlin", "Hotel Rehberge Berlin Mitte");
});
